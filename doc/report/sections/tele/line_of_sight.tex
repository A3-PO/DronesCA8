...
In Figure \ref{fig:GeometricDist_droneBasestation} it is shown that the two objects are a Ground Station and a drone. Both of them will not have an altitude bigger than approximately 100 meters and since \textit{R} is the radius of the Earth, $2hR$ >> $h^2$, and $h^2$ is therefore neglected in Equation \ref{eq:los_distToHorizon}. The two distances $D_D$ and $D_{GS}$ have the same expressions in both cases:
\begin{align*}
...
A simple but realistic example scenario would be where the aircraft is at $h_D = 100$m and the ground station at $h_GS = 20$m. This maximum distance between them is as follows:
\begin{equation*}
