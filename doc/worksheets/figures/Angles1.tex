...
%draw GROUND STATION
    \shade[shading=ball, ball color=blue] (0,0,0) circle (.3mm) node[above left] {\small{$(x_{gs},y_{gs},z_{gs})$}};

%draw DRONE
    \draw[-stealth,color=red] (O) -- (P) node[midway, above] {\small{$\rho$}};
    \shade[shading=ball, ball color=green] (P) circle (.3mm) node[above right] {\small{$(x_{d},y_{d},z_{d})$}};
